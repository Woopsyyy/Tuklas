...
const choiceAW = choiceW * (1592 / 1359) * 0.8 * 1.1 * 0.97;
  const choiceAH = choiceH * (916 / 771) * 0.8 * 1.1 * 0.97;
  ...
  const choiceATop = (choiceTop - choiceH * 0.0522) * 1.155 * 0.95 * 0.9 * 1.1 * 1.05;
  ...
  const text2Top = (bgOffsetY + 860 * bgScale - 0.10 * screenH - 0.10 * screenH + 0.20 * screenH) * 0.7 * 1.1 * 1.05 * 0.9;
  ...
  const nextSignLeft = Math.min(signLeft - 0.03 * screenW + signW * 1.19, screenW - signW - 24 * bgScale);
  const backSignLeft = Math.min(signLeft - 0.03 * screenW + signW * 1.26, screenW - signW * 0.95 - 24 * bgScale);
...
useCallback(() => {
  ...
  const handleSelectChoice = (choice: "A" | "B") => {
    ...
    setScore("antas3Level2", choice === "A");
    ...
    <View className="flex-1 bg-black">
      ...
      <View className="flex-1">
        ...
        <Animated.View entering={FadeInUp.duration(600)} style={{ position: "absolute", top: text1Top, left: text1Left, width: text1W, height: text1H, zIndex: 20 }} pointerEvents="none">
          <Image source={require("../../assets/images/antas3/question2/text1.png")} style={{ width: "100%", height: "100%" }} contentFit="contain"
            transition={0} />
        </Animated.View>
        ...
        <Animated.View entering={FadeIn.duration(600).delay(200)} style={{ position: "absolute", left: choiceALeft, top: choiceATop, width: choiceAW, height: choiceAH, zIndex: 40 }} pointerEvents="none">
          <Image source={require("../../assets/images/antas3/question2/A.png")} style={{ width: "100%", height: "100%" }} contentFit="contain"
            transition={0} />
        </Animated.View>
        ...
        <Animated.View entering={FadeIn.duration(600).delay(250)} style={{ position: "absolute", left: choiceBLeft, top: choiceTop, width: choiceW, height: choiceH, zIndex: 40 }} pointerEvents="none">
          <Image source={require("../../assets/images/antas3/question2/B.png")} style={{ width: "100%", height: "100%" }} contentFit="contain"
            transition={0} />
        </Animated.View>
        ...
        <Animated.View entering={FadeIn.duration(700).delay(100)} style={{ position: "absolute", top: text2Top, left: text2Left, width: text2W, height: text2H, zIndex: 20 }} pointerEvents="none">
          <Image source={require("../../assets/images/antas3/question2/text2.png")} style={{ width: "100%", height: "100%" }} contentFit="contain"
            transition={0} />
        </Animated.View>

        {/* Signpost Next */}
        <Animated.View style={{ position: "absolute", left: nextSignLeft, top: nextTop * 0.98 * 0.9 * 1.03 * 1.02 * 0.99, width: signW, height: nextH, zIndex: 25 }}>
          <Pressable onPress={handleNext} hitSlop={8} className="w-full h-full active:scale-95 active:opacity-80">
            <RNImage source={require("../../assets/images/ui/next.png")} fadeDuration={0} style={{ width: "100%", height: "100%" }} resizeMode="contain" />
          </Pressable>
        </Animated.View>

        {/* Signpost Back */}
        <Animated.View style={{ position: "absolute", left: backSignLeft, top: backTop * 1.02 * 0.9 * 1.03 * 1.01, width: signW * 0.95, height: backSignH * 0.95, zIndex: 25 }}>
          <Pressable onPress={handleBack} hitSlop={8} className="w-full h-full active:scale-95 active:opacity-80">
            <RNImage source={require("../../assets/images/ui/back-green.png")} fadeDuration={0} style={{ width: "100%", height: "100%" }} resizeMode="contain" />
          </Pressable>
        </Animated.View>
      </View>
    </View>
