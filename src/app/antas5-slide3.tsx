...
export default function Antas5Slide3Screen() {
  const router = useRouter();
  const { width, height } = useWindowDimensions();
  const insets = useSafeAreaInsets();
  const narrationVolume = useSettingsStore((state) => state.narrationVolume);
  const narration = useAudioPlayer(require("../../assets/audio/Kinausap.mp3"));
  ...
  // Source 128x56
  const sign2W = 700 * bgScale * 1.5 * 1.2 * 1.3 * 0.8 * 0.8 * 0.8 * 1.4;
  const sign2H = sign2W * (56 / 128);
  const sign2Left = (screenW - sign2W) / 2;
  const sign2Top = (screenH - sign2H) / 2 - 0.20 * screenH + 0.03 * screenH;
  ...
  const handleReplayNarration = async () => {
    try {
      ...
      console.warn("Antas5Slide3 narration replay error:", e);
    }
  };
  ...
  return (
    <View className="flex-1 bg-black">
      ...
      <View className="flex-1">
        ...
        {/* Text1 â€” centered in the middle */}
        <Animated.View
        ...
        {/* Next button â€” navigates to slide4 */}
        <Animated.View
          entering={FadeIn.duration(700)}
          style={{
            position: "absolute",
