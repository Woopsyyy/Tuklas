...
export default function Antas5Slide5Screen() {
  ...
  // Source 206x93
  const sign5W = 500 * bgScale * 1.5 * 1.5;
  const sign5H = sign5W * (93 / 206);
  const sign5Left = (screenW - sign5W) / 2;
  const sign5Top = (screenH - sign5H) / 2 - 60 * bgScale + 0.05 * screenH - 0.05 * screenH + 0.03 * screenH;
  ...
  useEffect(() => {
    ...
    return () => {
      try {
        ...
        console.warn("Antas5Slide5 narration cleanup error:", e);
      }
    };
  ...
  useEffect(() => {
    try {
      ...
      console.warn("Antas5Slide5 narration mute error:", e);
    }
  ...
  return (
    <View className="flex-1 bg-black">
      ...
      <SafeAreaView className="flex-1" edges={["top", "bottom", "left", "right"]}>
        ...
        {/* Next button — navigates to slide6 */}
        <Animated.View
          entering={FadeIn.duration(700)}
          style={{
            position: "absolute",
        ...
        >
          <Pressable
            onPress={() => router.navigate("/antas5-slide6")}
            hitSlop={8}
