...
const choiceAW = choiceW * (1592 / 1359) * 0.8 * 1.1 * 0.98;
  const choiceAH = choiceH * (916 / 771) * 0.8 * 1.1 * 0.98;
  const choiceGap = 20 * bgScale;
  const totalChoicesW = choiceW * 2 + choiceGap;
  const choiceTop = (bgOffsetY + 310 * bgScale + 0.05 * screenH + 0.10 * screenH + 0.10 * screenH) * 0.76 * 0.9;
  const choiceATop = (choiceTop - choiceH * 0.0522) * 1.155 * 0.9;
  ...
  const text2Top = (bgOffsetY + 860 * bgScale - 0.10 * screenH - 0.10 * screenH + 0.20 * screenH) * 0.7 * 1.1 * 1.1 * 0.9;
...
useCallback(() => {
  ...
  const handleSelectChoice = (choice: "A" | "B") => {
    ...
    setScore("antas3Level3", choice === "A");
